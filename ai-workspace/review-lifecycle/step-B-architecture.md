# Step 2 Â· Architecture Critique

> **Status:** ðŸŸ£ Conditional â€” `deep` keyword only  
...
Load when:
- Step 2 (Architecture Critique) is starting â€” ONLY if `deep` keyword is active
...
**Primary goal:** Challenge the architectural decisions in the PR. Identify wrong abstractions, hidden coupling, over-engineering, layer violations, and patterns that will create technical debt. Does NOT suggest rewrites â€” only raises questions and flags risks.

**Exit condition:** JSON output returned to Orchestrator â†’ proceed to Step 3 Â· Parallel Review. On failure â†’ log to `escalations[]`, continue without architecture findings.
...
|----------|----------------|
| Security-sensitive PR (auth, permissions, data access) | âœ… `deep` |
| Architecture-heavy change (new service, new pattern, breaking refactor) | âœ… `deep` |
| Large PR touching multiple subsystems | âœ… `deep` |
| Small bug fix or isolated UI change | âŒ Skip |
| Quick sanity check | âŒ Use `fast` instead |
...
| Role | Agent | Note |
|------|-------|------|
| **Architecture critic** | `gem-critic` | âœ… Installed. Read-only â€” challenges only. Never rewrites. |

---

## Invocation Prompt (Orchestrator â†’ `gem-critic`)
...
- Wrong abstraction layer (logic in the wrong place â€” e.g., business logic in a React component)
...
## What to read
Read changed files from the worktree â€” focus on structural decisions:
  ...
  "architecture_findings": [
    {
      ...
      "question": "The challenging question â€” e.g. 'Why is this in the component instead of the API client?'",
...
- If no architectural issues found, return empty findings array â€” do not fabricate issues
- Maximum 5 HIGH severity findings per review â€” be selective
```

---

## Output Contract (Step 2 â†’ Orchestrator)
...
```
On Step 2 success:
  â†’ store architecture_findings in memory context
  â†’ pass to Step 4 (Signal Filter) along with Step 3 findings
  â†’ set state.pipeline.critic = "done"

On Step 2 failure:
  â†’ log to state.escalations[]: "Step 2 (Architecture Critique) failed: {reason}"
  â†’ set state.pipeline.critic = "failed"
  â†’ continue to Step 3 WITHOUT architecture context
  â†’ note in report header: "âš ï¸ Architecture critique skipped (agent failed)"
```
...
| Failure | Policy |
|---------|--------|
| `gem-critic` fails | âš ï¸ Log, continue â€” architecture findings are valuable but not blocking |
| `gem-critic` returns empty findings | âœ… Valid â€” no issues found, proceed normally |
| `gem-critic` times out | âš ï¸ Same as failure â€” log and proceed |
