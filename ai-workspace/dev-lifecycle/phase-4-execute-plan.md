...
- `gem-implementer` is being invoked â€” need BUI constraints injection rules
- `gem-debugger` is conditionally triggered for a blocked task

> ðŸ“ **Context budget:** â‰¤ 8 000 tokens per task. Pass only the section relevant to current stream.
...
**Primary goal:** Implement tasks from the planning doc one at a time â€” each task fully done (code + unit test + doc update) before moving to the next.

> âš ï¸ **Test scope in Phase 4:** `gem-implementer` writes **unit tests inline with implementation** (TDD â€” verify the task works). Full test suite coverage (integration, E2E, 100% coverage audit) is Phase 7's responsibility.

**Exit condition:** All tasks done â†’ Phase 6. After each task â†’ Phase 5 (auto-trigger).
...
1. **Load task** â€” read planning doc, pick next `todo` task, present to user for confirmation
2. **Domain route** â€” check task tag:
   - `[fe]` tag AND `state.domain.has_frontend = true` â†’ **FE stream** (BUI-aware)
   - `[be]` tag or no tag â†’ **BE stream** (standard)
   - Both streams can run in parallel if they have no `conflicts_with` overlap
3. **Decompose** â€” delegate `gem-planner`: break task into ordered atomic sub-steps (max 5), each independently verifiable
4. **Gather context** â€” delegate `gem-researcher`: find relevant files, functions, patterns â€” no implementation
5. **Implement** â€” delegate `gem-implementer`:
   - **BE stream:** standard TDD â€” write failing test â†’ implement â†’ pass â†’ refactor
...
6. **Debug** *(if blocked)* â€” delegate `gem-debugger`: root-cause analysis â†’ fix proposal â†’ back to step 5
7. **Track** â€” delegate `lifecycle-scribe`: mark task done, record notes + deviations in planning doc â†’ triggers Phase 5

**Task queue statuses:** `todo` / `in-progress` / `done` / `blocked`

**Behavioral rules:**
- MUST execute one task at a time â€” NEVER start next task before current is `done`
- NEVER deviate from design doc without flagging as deviation in output JSON
- If blocked and Debugger cannot resolve â†’ mark as `blocked`, surface to user, move to next task
- Ask user after each section if new tasks were discovered â†’ add to planning doc

**Gates:**
- âš ï¸ Task blocked after 2 debug attempts â†’ escalate to user
- âš ï¸ Deviation from design doc â†’ flag in output, record in planning doc, continue
- âœ… All tasks `done` â†’ advance to Phase 6

---

## ðŸ¤– Agent Composition

> Pipeline runs **per task** â€” not per phase. `gem-debugger` is conditional (only if blocked). FE stream adds BUI skill injection to `gem-implementer`.

| Role | Agent | Status | Scope | Note |
|------|-------|--------|-------|------|
| **Task planner** | `gem-planner` | âœ… Installed | Decompose task into atomic sub-steps | DAG-based, max 5 sub-steps |
| **Context researcher** | `gem-researcher` | âœ… Installed | Find relevant files + patterns before implementing | No code changes â€” context only |
| **Implementer (BE stream)** | `gem-implementer` | âœ… Installed | TDD: write test â†’ implement â†’ pass â†’ refactor | Standard invocation |
| **Implementer (FE stream)** | `gem-implementer` | âœ… Installed | TDD + BUI constraints injected from design annotation | `[fe]` tasks only â€” extra context: BUI Design Constraints + coding-standards |
| **Debugger** | `gem-debugger` | âœ… Installed | Root-cause analysis when blocked | **Conditional** â€” only if blocked |
| **Doc tracker** | `lifecycle-scribe` | âœ… Installed | Mark task done + record notes in planning doc | Triggers Phase 5 after each task |
...
Do NOT implement anything â€” only gather and summarize context.
...
> `gem-implementer` â€” BE stream
```
You are being invoked as Implementer for feature {feature-name}, task: "{task-title}".

## Your Task
Implement this task using TDD: write failing test â†’ implement â†’ make test pass â†’ refactor.
...
> `gem-implementer` â€” FE stream *(only for `[fe]`-tagged tasks when `has_frontend: true`)*
```
You are being invoked as FE Implementer for feature {feature-name}, task: "{task-title}".

## Your Task
Implement this frontend task using TDD.
You MUST follow the BUI Design Constraints annotated in the design doc â€” no exceptions.
...
- No JSDoc in code files â€” document in README only
- Use direct imports, not barrel imports
- CSS Modules over makeStyles
- Remix Icons (`@remixicon/react`) â€” not `@material-ui/icons`
- MUI v7 components (Button, Chip, Card, Alert, Divider, IconButton) â†’ wrap in `<MuiV7ThemeProvider>` from `@internal/plugin-dop-common`
- Backstage components (InfoCard, LinkButton, Progress, Link) â†’ no wrapper needed
...
## Output Required
Updated planning doc written to disk (minimal diff â€” only change the relevant task checkbox and notes).
Return: { "tasks_remaining": N, "next_suggested_task": "..." }
```

---

## Output Contract (Phase-4 task â†’ Phase-5)
...
> Orchestrator **appends** each task's `perf` block to `state.metrics.phase_4[]` â€” Phase 4 is an array since it runs once per task.
