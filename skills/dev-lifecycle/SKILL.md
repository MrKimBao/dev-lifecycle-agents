...
| **6.5** | **Manual Verify** | *(gate — no doc)* | **Run app locally, test manually. Must pass before Phase 7.** |
| 7 | Write Tests | [references/writing-test.md](references/writing-test.md) | Add unit/integration tests — only after manual verify passes |
| 8 | Code Review | [references/code-review.md](references/code-review.md) | Final pre-push review |

> ⚠️ **Phase 6.5 gate**: Do NOT start Phase 7 until the user confirms manual verification has passed on the running app. If issues are found during manual verify → loop back to Phase 4.
...
- **Phase 6.5 (manual verify) finds bugs** → back to **Phase 4** to fix, then re-verify manually before Phase 7
...
**Language:** All `docs/ai/` content must be written in **English only** — regardless of the language used in the conversation. This includes headings, bullet points, descriptions, summaries, blockers, and progress notes. Never write Vietnamese or any other language in these files.
