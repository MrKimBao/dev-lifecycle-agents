...
**Next**: After completing any task → Phase 5 (Update Planning). When all tasks done → Phase 6 → **Phase 6.5 (Manual Verify)** → Phase 7 → 8.

> ⚠️ **Manual Verify gate (Phase 6.5)**: After Phase 6, ask the user to run the app locally and verify manually. Only proceed to Phase 7 (Write Tests) after the user explicitly confirms that manual testing passed. If bugs are found → loop back to Phase 4.

> ⚠️ **Language:** Write all doc updates in **English only**, even if the conversation is in another language.
